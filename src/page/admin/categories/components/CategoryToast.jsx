...
export default function CategoryToast({ toast, setToast }) {
  ...
  return (
    <div className={`fixed bottom-5 right-5 z-[200] max-w-sm w-full bg-white rounded-md shadow-xl border p-4 flex items-start gap-3 animate-in fade-in slide-in-from-bottom-5 duration-300 ${
      ...
      <button 
        ...
        className="text-admin-text-muted hover:text-admin-text-main flex-shrink-0 transition-colors p-1 hover:bg-admin-bg rounded-md border-0 bg-transparent cursor-pointer"
      >
        <X size={16} />
      </button>
    </div>
