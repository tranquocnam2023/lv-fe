...
export default function Header() {
  ...
  // State: selectedIndex - Vị trí sản phẩm được chọn bằng phím mũi tên
  const [selectedIndex, setSelectedIndex] = useState(-1);
  
  // References (useRef): cho Desktop & Mobile search bar
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          const normalized = data.map(p => ({
            ...
            name: p.name || p.Name,
            slug: p.slug || p.Slug,
            price: p.price || p.basePrice || p.BasePrice || 0,
            originalPrice: p.originalPrice || p.OriginalPrice || 0,
            image: p.image || p.thumbnailImage || p.ThumbnailImage || p.mainImage || p.MainImage
          }));
  ...
  // Đóng dropdown khi bấm ra ngoài vùng tìm kiếm (Kiểm tra cả desktop và mobile)
  useEffect(() => {
    // Hàm xử lý logic/sự kiện: handleClickOutside
    const handleClickOutside = (event) => {
      const isInsideDesktop = desktopSearchRef.current && desktopSearchRef.current.contains(event.target);
  ...
  // Hàm xử lý logic/sự kiện: handleKeyDown (Hỗ trợ phím mũi tên lên/xuống và Enter)
  const handleKeyDown = (e) => {
    const maxItems = Math.min(filteredProducts.length, 5);

    if (e.key === 'ArrowDown') {
      e.preventDefault();
      if (!showDropdown) setShowDropdown(true);
      if (maxItems > 0) {
        setSelectedIndex(prev => (prev < maxItems - 1 ? prev + 1 : 0));
      }
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      if (!showDropdown) setShowDropdown(true);
      if (maxItems > 0) {
        setSelectedIndex(prev => (prev > 0 ? prev - 1 : maxItems - 1));
      }
    } else if (e.key === 'Enter') {
      if (showDropdown && selectedIndex >= 0 && selectedIndex < maxItems && filteredProducts[selectedIndex]) {
        e.preventDefault();
        const selectedProd = filteredProducts[selectedIndex];
        const target = selectedProd.slug || selectedProd.id || selectedProd.Slug || selectedProd.Id;
        setShowDropdown(false);
        setSearchQuery('');
        setSelectedIndex(-1);
        if (target) {
  ...
  return (
    <header className="w-full text-white sticky top-0 z-50 shadow-md" style={{ backgroundColor: THEME.primary, color: THEME.textLight }}>
      ...
      <div className="container-box flex items-center justify-between py-3 h-16 px-4 md:px-6 lg:px-8 relative">
        ...
        {/* DESKTOP: Search Bar */}
        <div className="hidden lg:block flex-1 max-w-xl mx-4 min-w-[200px]">
          <HeaderSearchBar
            searchContainerRef={desktopSearchRef}
      ...
      <div className="block lg:hidden px-4 pb-3">
        <HeaderSearchBar
          searchContainerRef={mobileSearchRef}
          searchQuery={searchQuery}
          setSearchQuery={setSearchQuery}
          setShowDropdown={setShowDropdown}
          handleKeyDown={handleKeyDown}
